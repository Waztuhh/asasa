...
const toplams = require("../../mongoo/toplams");
...
const isimler = require("../../mongoo/names");
const regstats = require("../../mongoo/registerStats");

const otokayit = require("../../mongoo/otokayit");
...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kayıt <@Yousef/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
...
Üyesinin önceki isimlerine \`.isimler <@Yousef/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
    `)
...
.setFooter(`Lütfen 30 saniye alttaki butonlara basarak kullanıcının cinsiyetini belirleyin.`)
 ...
 let msg = await message.channel.send({ buttons : [ button_1, button_2, button_3 ], embed: Yousef})
...
msg.edit({components: null, embed: Yousefe}); 
...
msg.edit({components: null, embed: Yousefk}); 
