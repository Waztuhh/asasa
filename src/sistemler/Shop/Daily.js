...
const { altin, altin2, rewards } = require("../../ayarlar/emojiler.json")
...
run: async (client, message, args) => {
   ...
   if (!message.guild) return;
	...
	let data = await Yousef.findOne({userID: message.author.id, guildID: message.guild.id});
  if(!data) return message.lineReply(`Hey! Günlük hediye almak ve oyunlarımızı oynamak için coin hesabı oluşturman gerekiyor.\n**!hesapoluştur** yazarak kendi hesabını oluşturabilirsin.`)
    ...
    if (gunluk !== null && timeout - (Date.now() - gunluk) > 0) {
        ...
        await Yousef.findOneAndUpdate({userID: message.author.id, guildID: message.guild.id}, {$inc: {dolar: sayi}, $set: {dolarTime: Date.now()}}, {upsert: true})
