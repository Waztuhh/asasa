...
let ayar = require("../../ayarlar/sunucuayarlar.json"); 
let sunucu = require("../../ayarlar/botayarlar.json");  
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ buttons : [ecdagit,tagrol,kayıtsızdagit], embed: Yousef})
    ...
    let Yousef = message.guild.members.cache.filter(member => !member.roles.cache.has(ayar.etkinlik) || !member.roles.cache.has(ayar.cekilis))
    button.reply.send(`
Etkinlik/Çekiliş rolü olmayan ${Yousef.size} kullanıcıya etkinlik, çekiliş rolleri verildi ! `)
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`)
    ...
    button.reply.send(`
Kayıtsız rolü olmayan ${Yousefcim.size} kullanıcıya kayıtsız rolü verildi !

Kayıtsız Rolü verilen kullanıcılar;
${Yousefcim.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `)
