...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const Yousefcik = await message.lineReply(
        `${members
            .map((member, idx) => `**${idx + 1}. ${member.toString()}**`)
            .join("\n")}\nBu üyeleri banlamak istiyor musun?`,
    );
    await Yousefcik.react(green);
    const collector = Yousefcik.createReactionCollector(
        (reaction, user) =>
    ...
    collector.on("collect", async () => {
        await Yousefcik.edit(`${green} ${members.length} adet kullanıcı başarıyla yasaklandı.`);
    ...
    collector.on("end", (_, reason) => {
        ...
        if (reason === "time")
            Yousefcik.edit("10 saniye geçtiği için işlem iptal edildi.");
    });
